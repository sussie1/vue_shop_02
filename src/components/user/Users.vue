<template>
  <div>
    ...
    <!-- 分配角色对话框 -->
    <el-dialog title="分配角色" :visible.sync="setRoleDialogVisble"  width="50%" @close=setRoleDialdogClosed>
      <!-- 内容主体区域 -->
      <div>
        <p>当前用户名:{{this.userInfo.username}}</p>
        <p>当前角色名:{{this.userInfo.role_name}}</p>
        <p>选择新的角色:
...
export default {
  data () {
    ...
    return {
      ...
      // 表单验证规则
      addFormRules: {
        username: [{ required: true, message: '请输入用户名', trigger: 'blur' }, { min: 3, max: 5, message: '请输入3-5个字符', trigger: 'blur' }],
        password: [{ required: true, message: '请输入密码', trigger: 'blur' }, { min: 6, max: 12, message: '请输入6-12个字符', trigger: 'blur' }],
        email: [{ required: true, message: '请输入邮箱', trigger: 'blur' }, { validator: checkEmail, message: '邮箱格式不正确，请重新输入', trigger: 'blur' }],
        mobile: [{ required: true, message: '请输入手机号码', trigger: 'blur' }, { validator: checkMobile, message: '手机号码不正确，请重新输入', trigger: 'blur' }]
      },
      // 控制编辑用户信息对话框的显示与隐藏
  ...
  methods: {
    ...
    handleSizeChange (newSize) {
      ...
      console.log('每页' + newSize + '条数')
      this.queryInfo.pagesize = newSize
      // 重新按照pagesize发送请求，请求最新的数据
      this.getUserList()
    },
    // 控制当前的页码
    // currentpage改变时触发，当currentpage发生改变的时候，我们应该
    // 以最新的currentpage来请求数据并展示数据
    ...
    editUser (id) {
      ...
      this.$refs.editFormRef.validate(async valid => {
        // console.log(valid)
        if (!valid) return this.$message.error('填写正确的信息')
        const { data: res } = await this.$http.put('users/' + id, { email: this.editForm.email, mobile: this.editForm.email })
        // console.log(res)
    ...
    // 保存提交更新的角色名称保存到userInfo
    async saveRole () {
      if (!this.selectRoleId) return this.$message.error('请选择要分配角色')
